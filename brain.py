...
def add_engine(url, short_url):
    check_db_health()
    c.execute("INSERT INTO url VALUES (:original, :raccourci, :heure)",
        {
            'original': url,
            'raccourci': short_url,
            'heure': time_cal()
...
def check_engine(url):
    check_db_health()
    c.execute('SELECT * FROM url WHERE original = :original', {
        'original': url
    })
...
def check_duplicate(short_url):
    check_db_health()
    c.execute('SELECT * FROM url WHERE raccourci = :raccourci', {
        'raccourci': short_url
    })
    raw_db_content = c.fetchall()
    if raw_db_content == []:
...
def debug_engine():
    check_db_health()
    c.execute('SELECT rowid, * FROM url')
    raw_db_content = c.fetchall()
    print("INDEX \t   Original \t\t Raccourcis \t\t Heure")
